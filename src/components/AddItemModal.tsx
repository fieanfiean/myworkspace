...
export function AddItemModal(props: Props) {
  ...
  const required: Record<Props['kind'], string[]> = { experience: ['title', 'organization', 'dateRange'], education: ['degree', 'school', 'dateRange'], skill: ['categoryId', 'name', 'level'], achievement: ['category', 'title', 'year', 'rank'] };
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      else await props.onSubmit({ category: values.category as AchievementCategory, title: values.title, year: values.year, tag: values.tag ?? '', rank: values.rank, imageUrl: values.imageUrl ?? '' });
      props.onClose();
    } catch (reason) {
