...
function ResumeDocument({ data: sourceData, payload, selected, kind }: { data: AboutMeData; payload: ExportPayload; selected: Record<SectionKey, boolean>; kind: ExportKind }) {
  ...
  const certifications = sourceData.achievements.filter(item => item.category === 'certification').slice(0, compact ? 2 : undefined);
  ...
  return (
    <article style={{ width: 794, minHeight: 1123, height: compact ? 1123 : undefined, overflow: compact ? 'hidden' : 'visible', boxSizing: 'border-box', background: '#fff', color: '#334155', padding: compact ? '35px 45px' : '54px 60px', fontFamily: 'Arial, sans-serif', fontSize: compact ? 10 : 12, lineHeight: compact ? 1.3 : 1.5 }}>
      ...
      {selected.achievements && projects.length > 0 && <section><h2 style={sectionTitle}>Projects</h2>{projects.map(item => <div key={item.id} style={itemStyle}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 20 }}><strong style={{ color: '#0f172a', fontSize: 14 }}>{item.title}</strong><span style={{ color: '#64748b' }}>{item.year}</span></div><span>{[item.rank, item.tag].filter(Boolean).join(' · ')}</span></div>)}</section>}

      {selected.achievements && awards.length > 0 && <section><h2 style={sectionTitle}>Awards &amp; Honors</h2>{awards.map(item => <div key={item.id} style={itemStyle}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 20 }}><strong style={{ color: '#0f172a', fontSize: 14 }}>{item.title}</strong><span style={{ color: '#64748b' }}>{item.year}</span></div><span>{[item.rank, item.tag].filter(Boolean).join(' · ')}</span></div>)}</section>}
      ...
      {selected.certifications && (certifications.length > 0 || certificationRecords.length > 0) && <section><h2 style={sectionTitle}>Certifications &amp; Workshops</h2>{certifications.map(item => <div key={item.id} style={itemStyle}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 20 }}><strong style={{ color: '#0f172a', fontSize: 14 }}>{item.title}</strong><span style={{ color: '#64748b' }}>{item.year}</span></div><span>{[item.rank, item.tag].filter(Boolean).join(' · ')}</span></div>)}{certificationRecords.map((item, index) => <div key={value(item, 'id') || index} style={itemStyle}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 20 }}><strong style={{ color: '#0f172a', fontSize: 14 }}>{value(item, 'name', 'title')}</strong><span style={{ color: '#64748b' }}>{value(item, 'year', 'date', 'issued_at')}</span></div><span style={{ color: '#2563eb' }}>{value(item, 'issuer', 'organization')}</span></div>)}</section>}
    </article>
  );
}
...
export function ExportPanel({ data }: ExportPanelProps) {
  ...
  const exportDocument = async (kind: ExportKind) => {
    ...
    try {
      ...
      const [publicationResult, certificationResult] = await Promise.all([
        selected.publications ? supabase.from('publications').select('*').eq('profile_id', user.id) : Promise.resolve({ data: [] }),
        selected.certifications ? supabase.from('certifications').select('*').eq('profile_id', user.id) : Promise.resolve({ data: [] }),
      ]);
  ...
  return <>
    <aside className="export-panel-container">
      <div><h3 className="mb-6 text-xl font-bold text-white">{t('exportPanel.title')}</h3><div className="drop-zone mb-6"><Upload className="mx-auto mb-2 text-blue-500" size={24} /><p className="text-sm font-semibold text-slate-200">{t('exportPanel.dropZoneText')}</p><p className="mt-1 text-xs text-slate-500">{t('exportPanel.dropZoneSubtext')}</p></div>
    ...
    {payload && <div aria-hidden="true" style={{ position: 'fixed', left: -10000, top: 0, zIndex: -1 }}><div ref={documentRef}><ResumeDocument data={data} payload={payload} selected={selected} kind={exportKind} /></div></div>}
  </>;
}
