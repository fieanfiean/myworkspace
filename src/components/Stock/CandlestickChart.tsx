...
export function CandlestickChart({ ticker, priceHistory }: { ticker: string; priceHistory: PricePoint[] }) {
  ...
  const chart = useMemo(() => {
    ...
    return { width, height, x, y, maPath, min, max };
  }, [priceHistory]);
  ...
  return <section className="rounded-2xl border border-slate-800 bg-[#111827] p-4 shadow-xl shadow-black/10 sm:p-5">
    ...
    <div className="mt-3 overflow-x-auto" data-horizontal-scroll><svg viewBox={`0 0 ${chart.width} ${chart.height}`} className="h-[300px] min-w-[720px] w-full" role="img" aria-label={t('stock.candlestickLabel')}>
