...
export function BudgetPage({ toolsOpen, onCloseTools }: { toolsOpen: boolean; onCloseTools: () => void }) {
  ...
  const transactionSentinelRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    if (!sentinel || loading || visibleTransactionCount >= filteredTransactions.length) return;
    const observer = new IntersectionObserver(([entry]) => {
      if (!entry?.isIntersecting) return;
      observer.disconnect();
      setTransactionPage({ filterKey: transactionFilterKey, count: visibleTransactionCount + transactionPageSize });
    }, { rootMargin: '0px 0px 200px 0px' });
    observer.observe(sentinel);
    return () => observer.disconnect();
  }, [filteredTransactions.length, loading, transactionFilterKey, visibleTransactionCount]);
