...
import { ArrowDownRight, ArrowUpRight, Banknote, CalendarDays, Car, CircleDollarSign, Clapperboard, HeartPulse, LoaderCircle, Plus, ReceiptText, ShoppingCart, Utensils, WalletCards, Zap } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { Area, AreaChart, CartesianGrid, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
import { useAuth } from '@/hooks/useAuth';
import { useBudgetTransactions } from '@/hooks/useBudgetTransactions';
import type { NewBudgetTransaction, TransactionCategory, TransactionType } from '@/types/budget';
...
export function BudgetPage() {
  ...
  const { user } = useAuth();
  const { transactions, loading, error, addTransaction } = useBudgetTransactions(user?.id);
  const [form, setForm] = useState<NewBudgetTransaction>({ type: 'expense', amount: 0, description: '', transactionDate: today(), category: 'groceries' });
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!Number.isFinite(form.amount) || form.amount <= 0 || !form.description.trim()) { setFormError(t('budget.form.validation')); return; }
    setSaving(true); setFormError(null);
    try { await addTransaction(form); setForm(current => ({ ...current, amount: 0, description: '', transactionDate: today() })); }
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
      <div className="min-w-0 space-y-5">
        <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5 shadow-xl shadow-black/10">
          ...
          <div className="h-72 w-full" aria-label={t('budget.chart.title')}><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData} margin={{ top: 8, right: 8, left: -10, bottom: 0 }}>
            ...
            <CartesianGrid stroke="#1e293b" vertical={false} strokeDasharray="4 4"/><XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 12 }} dy={10}/><YAxis axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 12 }} tickFormatter={(value: number) => `$${value / 1000}k`}/><Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 12 }} labelStyle={{ color: '#f8fafc' }} formatter={value => currency.format(Number(value))}/><Area type="monotone" dataKey="income" name={t('budget.income')} stroke="#34d399" strokeWidth={2.5} fill="url(#incomeFill)"/><Area type="monotone" dataKey="expenses" name={t('budget.expense')} stroke="#fb7185" strokeWidth={2.5} fill="url(#expenseFill)"/>
          </AreaChart></ResponsiveContainer></div>
        </section>

        <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/80 shadow-xl shadow-black/10"><div className="flex items-center justify-between border-b border-slate-800 px-5 py-4"><h2 className="font-semibold text-white">{t('budget.recent.title')}</h2><span className="text-xs text-slate-500">{t('budget.recent.count', { count: transactions.length })}</span></div><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-left">
          <thead className="bg-slate-950/40 text-[11px] uppercase tracking-wider text-slate-500"><tr>{(['date','description','category','amount'] as const).map(key => <th key={key} className={`px-5 py-3 font-medium ${key === 'amount' ? 'text-right' : ''}`}>{t(`budget.recent.${key}`)}</th>)}</tr></thead>
          <tbody className="divide-y divide-slate-800/80">{loading ? <tr><td colSpan={4} className="px-5 py-12 text-center text-sm text-slate-500"><LoaderCircle className="mx-auto mb-2 animate-spin" size={20}/>{t('budget.loading')}</td></tr> : transactions.length === 0 ? <tr><td colSpan={4} className="px-5 py-12 text-center text-sm text-slate-500">{t('budget.recent.empty')}</td></tr> : transactions.slice(0, 8).map(item => { const style = categoryStyle[item.category], Icon = style.icon; return <tr key={item.id} className="transition hover:bg-slate-800/35"><td className="whitespace-nowrap px-5 py-4 text-sm text-slate-400">{new Intl.DateTimeFormat(locale, { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' }).format(new Date(`${item.transactionDate}T00:00:00Z`))}</td><td className="px-5 py-4 text-sm font-medium text-slate-200">{item.description}</td><td className="px-5 py-4"><span className={`inline-flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-xs font-medium ${style.classes}`}><Icon size={14}/>{t(`budget.categories.${item.category}`)}</span></td><td className={`whitespace-nowrap px-5 py-4 text-right text-sm font-semibold ${item.type === 'income' ? 'text-emerald-400' : 'text-rose-300'}`}>{item.type === 'income' ? '+' : '-'}{currency.format(item.amount)}</td></tr>; })}</tbody>
        </table></div></section>
      </div>
      ...
      <aside className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5 shadow-xl shadow-black/10 xl:sticky xl:top-6"><div className="mb-5 flex items-center gap-3"><span className="rounded-xl bg-indigo-500/15 p-2 text-indigo-400"><Plus size={19}/></span><div><h2 className="font-semibold text-white">{t('budget.form.title')}</h2><p className="text-xs text-slate-500">{t('budget.form.subtitle')}</p></div></div>
        <form className="space-y-5" onSubmit={event => void submit(event)}>
          ...
          <label className="block"><span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-slate-500">{t('budget.form.amount')}</span><span className="flex overflow-hidden rounded-xl border border-slate-700 bg-slate-950 focus-within:border-indigo-500"><span className="border-r border-slate-800 px-3 py-2.5 text-slate-500">$</span><input required min="0.01" step="0.01" type="number" value={form.amount || ''} onChange={event => setForm(current => ({ ...current, amount: event.target.valueAsNumber }))} placeholder="0.00" className="min-w-0 flex-1 bg-transparent px-3 text-sm text-white outline-none"/></span></label>
