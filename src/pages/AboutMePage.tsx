  ...
  return <>
    ...
    <div className="mx-auto flex max-w-7xl items-start gap-8">
      <main className="min-w-0 flex-1 space-y-4">
        <ProfileHeader/>
        <ExperienceList items={data.experiences} onAdd={() => setEditor({ kind: 'experience' })} onEdit={item => setEditor({ kind: 'experience', item })} onDelete={id => remove('experiences', id)}/>
        <EducationSection items={data.educations} onAdd={() => setEditor({ kind: 'education' })} onEdit={item => setEditor({ kind: 'education', item })} onDelete={id => remove('educations', id)}/>
